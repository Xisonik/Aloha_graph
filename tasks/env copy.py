...
class CLGRCENV(gym.Env):
    ...
    def __init__(
        ...
        gym.Env.__init__(self)
        self.action_space = spaces.Box(low=-1, high=1.0, shape=(2,), dtype=np.float32)
        self.observation_space = spaces.Box(low=-1000000000, high=1000000000, shape=(2054,), dtype=np.float32)
    ...
    def move(self, action):
        ...
        if not self.demonstrate:
            ...
            forward_velocity, angular_velocity = self.get_demonstration_move()
        for i in range(self._skip_frame):
            self.jetbot.apply_wheel_actions(
                self.jetbot_controller.forward(command=[forward_velocity, angular_velocity])
            )
    ...
    def get_demonstration_move(self):
        v, w = 0, 0
        goal_world_position = self.goal_position
        goal_world_position[2] = 0
        ...
        orientation_error = delta_angle if delta_angle < 180 else (360 - delta_angle)
        # print("delta_angle", quadrant)
        w =  sign * min(orientation_error/10, 1.7)
        v = min(v_g,0.8) - min(abs(w/5), 0.7)
        # print("v_g, v_o = ", v_g, v_o)
        # print("current_dist_to_goal = ", current_dist_to_goal)
        # print("current_dist_to_obs = ", current_dist_to_obs)
        # print("to_goal_vec = ", to_goal_vec)
        # print("to_obs_vec = ", to_obs_vec)
    ...
    def get_position(self, x_goal, y_goal):
        ...
        reduce_phi = 1
        track_width = 1.2
        pos_obstacles = np.array([[3.9,-0.6],[4,1]])
        if self.change_line >= self.repeat:
            reduce_r = np.random.rand()
            reduce_phi = np.random.rand()
            self.change_line=0
        ...
        while 1:
            ...
            quadrant = self.get_quadrant(nx, ny, to_goal_vec)
            if target_pos[1]>=-2.3 and target_pos[1]<=2.3 and target_pos[0]>=2.2 and ((target_pos[1]<=-0 and target_pos[0]>-target_pos[1]*0.572+2.35)
                        or (target_pos[1]>=0 and target_pos[0]>target_pos[1]*0.285+2.35)) and (np.abs(target_pos[1] - goal_world_position[1]) < track_width) and self.no_intersect_with_obstacles(target_pos[0:2],pos_obstacles,0.75):
                n = np.random.randint(2)
                return target_pos, quadrant*np.arccos(cos_angle) + ((-1)**n)*reduce_phi*self.traning_angle
            elif k >= 100:
                print("can't get correct robot position: ", target_pos, quadrant*np.arccos(cos_angle) + reduce_phi*self.traning_angle, reduce_r, self.num_of_envs)
    ...
    def get_observations(self):
        ...
        if images is not None:
            ...
            img = images/255
            # if self._get_current_time() < 20:
            # save_image(make_grid(img, nrows = 2), '/home/kit/.local/share/ov/pkg/isaac-sim-2023.1.1/standalone_examples/Aloha_graph/Aloha/img/memory.png')
        else:
            print("Image tensor is NONE!")
        transform = T.ToPILImage()
        is_segmentation = asdict(self.config).get('segmentation', None)
        if is_segmentation:
            yimg = [images[0].cpu().numpy().transpose(1, 2, 0), images[1].cpu().numpy().transpose(1, 2, 0)]
            yolo_classes = list(self.model.names.values())
            classes_ids = [yolo_classes.index(clas) for clas in yolo_classes]
            conf = 0.3
            results = [self.model.predict(yimg[0], classes=45, conf=conf), self.model.predict(yimg[1], classes=45, conf=conf)]
            if results[0][0].masks is not None:
                colors = 255#[random.choices(range(256), k=1) for _ in classes_ids]
                for result in results[0]:
                    for mask, box in zip(result.masks.xy, result.boxes):
                        points = np.int32([mask])
                        color_number = classes_ids.index(int(box.cls[0]))
                        cv2.fillPoly(yimg[0], points, [0,256,0])
                # cv2.imwrite("/home/kit/.local/share/ov/pkg/isaac-sim-4.1.0/standalone_examples/Aloha_graph/yolotest_1.png", yimg[0])
                img_current_0 = self.clip_preprocess(transform(yimg[0])).unsqueeze(0).to(self.device)
            else:
                print("can't detect lefr")
                # cv2.imwrite("/home/kit/.local/share/ov/pkg/isaac-sim-4.1.0/standalone_examples/Aloha_graph/yolotest_1.png", yimg[0])
                img_current_0 = self.clip_preprocess(transform(img[0])).unsqueeze(0).to(self.device)

            if results[1][0].masks is not None:
                colors = 255#[random.choices(range(256), k=1) for _ in classes_ids]
                for result in results[1]:
                    for mask, box in zip(result.masks.xy, result.boxes):
                        points = np.int32([mask])
                        color_number = classes_ids.index(int(box.cls[0]))
                        cv2.fillPoly(yimg[1], points, [0,256,0])
                # cv2.imwrite("/home/kit/.local/share/ov/pkg/isaac-sim-4.1.0/standalone_examples/Aloha_graph/yolotest_2.png", yimg[1])
                img_current_1 = self.clip_preprocess(transform(yimg[1])).unsqueeze(0).to(self.device)
            else:
                print("can't detect")
                # cv2.imwrite("/home/kit/.local/share/ov/pkg/isaac-sim-4.1.0/standalone_examples/Aloha_graph/yolotest_2.png", yimg[1])
    ...
    def _on_contact_report_event(self, contact_headers, contact_data):
        ...
        for contact_header in contact_headers:
            ...
            for index in range(contact_data_offset, contact_data_offset + num_contact_data, 1):
                ...
                cur_impulse = math.sqrt(cur_impulse)
            pos_obstacles = np.array([[3.9,-0.6],[4,1]])
            if num_contact_data > 1 or not self.no_intersect_with_obstacles(self.current_jetbot_position,pos_obstacles,0.63): #1 contact with flore here yet
                self.collision = True